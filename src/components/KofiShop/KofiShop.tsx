    ...
    return (
        <a href={`http://ko-fi.com/s/${Alias}`} className="shop-item">
            <div className="shop-item-thumbnail">
                <img loading="lazy" src={`https://storage.ko-fi.com/cdn/useruploads/post/${ThumbnailUrls[0]}`} className="shop-item-thumbnail" />
...
export const KofiShopGrid = (props: KofiShopGridProps) => {
    return (<div id="kofi-item-grid" className="shop_base_colorscheme"> {props.inventory.map((item, index) =>
        <KofiShopCard key={index} Alias={item.Alias} Name={item.Name} Price={item.Price} ThumbnailUrls={item.ThumbnailUrls} Description={item.Description} />)}
    </div>);
}
